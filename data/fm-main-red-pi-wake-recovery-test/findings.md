...
- The hosted CI rerun on the first production fix exposed the inner failure after diagnostic output was added: Node 22.23.2 saw only `arm=... | arm=...`, meaning the shell-based hung fixture's SIGTERM path itself was not deterministic under the hosted runner.
- The fixture now uses a direct Node successor with an explicit SIGTERM handler while retaining the inherited-pipe holder. Node 22.23.2 passes the complete focused Pi/OpenCode suite locally with this fixture.
