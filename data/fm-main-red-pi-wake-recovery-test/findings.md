...
- A 20-run Node 22.23.2 focused loop passed 20/20, and an eight-CPU-burner Node 22.23.2 loop passed 50/50. Therefore the reported Pi failure is environment-specific in the available local environment, while the hosted serial failure was reproducible across the inspected CI reruns.
...
- The hosted CI rerun `31925623983` still saw only two arm rows under Node 22.23.2. Temporary lifecycle tracing showed `shell-start` and `holder-start` but no `node-start`, `sigterm`, or `node-exit`: the direct-Node diagnostic fixture itself was racing the unchanged 250ms readiness budget and never reached its intended arm process before retirement. This was an independent fixture-startup mask, not evidence against the process-exit fix.
- The fixture now starts `sleep 60` immediately after forking the inherited-pipe holder. This keeps the counterfactual (arm process exits while a descendant retains stdout) without adding a Node startup race. Node 22.23.2 passes the complete focused Pi/OpenCode suite with this deterministic fixture.
- Hosted CI run `31927133010` passed, including `Behavior portable serial 4` and all sibling behavior suites. This is the first hosted pass after the deterministic fixture and exit-based repair were both present. The earlier hosted run `31926361985` still showed one arm row with the prior sleep fixture but without lifecycle diagnostics; the final passing run confirms the fix and fixture together.
