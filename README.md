...
Some harnesses use the slash form shown here; others use the same names with `$`, such as `$afk`.
...
| `/explain`         | Plain-language visual explainer for any report or concept — resolves the subject, delegates meaty topics to a spawned explainer mate that builds a visual Lavish page, and relays follow-up questions; never replaces normal chat reporting |
