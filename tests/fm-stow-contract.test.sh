...
test_stow_skill_task_note_contract() {
  ...
  assert_grep 'bin/fm-tasks-axi.sh show <id> --full' "$stow" "stow skill does not require inspecting task notes first"
  assert_grep 'bin/fm-tasks-axi.sh update <id> --body-file <path>' "$stow" "stow skill does not require task body replacement"
...
test_agents_backlog_task_note_contract() {
  ...
  assert_no_grep 'bin/fm-tasks-axi.sh show <id> --full' "$agents" \
    "AGENTS.md duplicates exact task-note read syntax from its conditional owner"
  assert_no_grep 'bin/fm-tasks-axi.sh update <id> --body-file <path>' "$agents" \
  ...
  assert_no_grep 'tasks-axi show <id> --full' "$agents" \
    "AGENTS.md duplicates short task-note read syntax from its conditional owner"
  assert_no_grep 'tasks-axi update <id> --body-file <path>' "$agents" \
    "AGENTS.md duplicates short task-note update syntax from its conditional owner"
