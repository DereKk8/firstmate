...
family_for_basename() {
  case "$1" in
    ...
    fm-documentation-audiences.test.sh|fm-ensure-agents-md.test.sh|fm-external-tooling-check.test.sh|\
    fm-grok-harness.test.sh|fm-kimi-harness.test.sh|fm-herdr-lab.test.sh|fm-lint.test.sh|\
...
families_for_changed_path() {
  ...
  case "$path" in
    ...
    bin/fm-install-herdr.sh|bin/fm-install-treehouse.sh|bin/fm-herdr-ci-cleanup.sh)
      ...
      printf '%s\n' real-herdr-gated
      ;;
    bin/fm-lint.sh|bin/fm-install-shellcheck.sh|bin/fm-external-tooling-check.sh|\
