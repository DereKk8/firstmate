...
`complete --none` records a `decision_none_status=<fingerprint>` meta line for the exact default decision event reviewed at that point. The fingerprint includes the event's append-only status-line position, so a later `needs-decision:` event is not covered by an earlier clean review. Both `complete` and `verify` compare the current default event with that recorded fingerprint.
The fingerprint is set only by an explicit `--none` call and is never inferred. A genuinely new default decision therefore refuses completion even when the origin was previously reviewed cleanly.
...
- `attest <origin-id> <decision-key> --decision-file <path> --note <text> [--routed-to <id>...]` records a resolution for a hold already `state: done`, `kind: captain`, closed outside this script (for example by a bare `tasks-axi done`). Each named dependent must have the hold's raw `blocked-by` edge in its durable `deps` field from before the close; attest never creates a new edge. An exact retry with the same decision digest, note, and routed identities succeeds, while changed evidence is refused. The body marker is `Resolution recorded by fm-decision-hold. (attested; hold closed outside fm-decision-hold prior to attest)`.
- `amend <origin-id> <decision-key> --decision-file <path> --note <text> [--routed-to <id>...]` (re)writes the resolution record for a hold already `state: done`, `kind: captain`, whether the record is missing (an ordinary `tasks-axi update --body` on a resolved hold silently strips it, since `resolve` only ever writes the attestation into that same mutable field) or present but wrong (the captain corrected an earlier ruling). It requires at least one routed dependent with a pre-existing raw edge or a route-evidence record from an earlier `resolve`; it may re-establish only a route covered by that evidence. It always overwrites and always requires `--note`. The body marker is `Resolution recorded by fm-decision-hold. (amended)`.
- `supersede <origin-id> <decision-key> --duplicate-of <hold-id> --note <text>` retires a duplicate hold by pointing it at an already durable authoritative peer (`verify_hold_durable`: actively held or resolved). A superseded peer is followed to its active or resolved authority, and cycles are refused, so two unanswered holds cannot attest each other. The body marker is `Superseded by fm-decision-hold.` with `Duplicate of: <hold-id>`.

`resolve` writes one durable route-evidence file under `data/decision-hold-routes/` after validating each active dependency and before clearing any edge. Repair commands consult that trace, or the dependent's raw `deps` edge when an external close left it behind; they never manufacture a dependency as proof. `verify_hold_durable` (the only check the completion gate and teardown actually run) accepts a structurally complete `Resolution recorded by fm-decision-hold.` record with a nonempty decision digest and routed identity set, or follows a `Superseded by fm-decision-hold.` reference to an active or structurally resolved authority. Invalid references and cycles fail closed.
...
Retention-deadlock repair (event-scoped `--none` coverage, `attest`, `amend`, `supersede`) verification date: 2026-07-23.

### Retention-deadlock repair regression

The focused regression covers the retention-deadlock repairs with synthetic `sample` identities: a reviewed default event followed by a genuinely new default event; a hold closed with plain `tasks-axi done` instead of `resolve`; an ordinary `tasks-axi update --body` on an already-resolved hold that silently strips the attestation; and superseded-peer, resolved-peer, and two-hold supersede-cycle graphs.
The repair cases assert that original routed dependents can be preserved or explicitly re-established only from durable evidence, an unrelated task named after an external close is refused, empty routed sets are refused, exact attest retries succeed, changed attest evidence fails, and the existing close-ordering invariant remains intact.
...
ok - an unreviewed default decision still refuses --none completion; the reviewed-event fingerprint is not a blanket bypass
ok - --none coverage is scoped to the reviewed default status event
ok - attest repairs a hold closed outside the tool with an evidenced, distinguishable, idempotent record
ok - supersede retires a duplicate hold against a durable authoritative peer, active or resolved
ok - supersede rejects cycles and follows only active or genuinely resolved authority
ok - repair paths preserve, restore, and require routed dependent identities
