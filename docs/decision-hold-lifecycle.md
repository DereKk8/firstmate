...
`verify_hold_durable` (the only check the completion gate and teardown actually run) accepts a structurally complete `Resolution recorded by fm-decision-hold.` record with a nonempty decision digest and routed identity set, or follows a `Superseded by fm-decision-hold.` reference to an active or structurally resolved authority. Invalid references and cycles fail closed.
...
The repair cases assert that original routed dependents can be preserved or explicitly re-established, empty routed sets are refused, exact attest retries succeed, changed attest evidence fails, and the existing close-ordering invariant remains intact.
