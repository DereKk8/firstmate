...
The script header owns the exact run-head ancestry rules; an active pipeline-owned run advanced past the local head remains attributable through its branch_sync submitted-head binding to that same local head, without weakening rewrite/divergence protection.
