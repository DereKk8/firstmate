...
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` compares against the project's default branch (fetching `origin/<default>` for remote-backed projects, the local default for local-only ones); the compare side fetches and compares against `refs/pull/<n>/head` by default when a PR is known (recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
