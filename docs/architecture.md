...
When a selected delivery path calls for a diff, `bin/fm-review-diff.sh` resolves the project's base branch from the registry by exact path identity (`bin/fm-project-base.sh --path`) and refuses loudly when no registry entry matches, then prefers the forge's actual PR base when a PR is known; the compare side fetches and compares against `refs/pull/<n>/head` by default (recorded `pr_head=` is only an offline fallback) before falling back to the local branch with a warning.
