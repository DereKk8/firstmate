...
`tests/fm-pr-body-check.test.sh` is a later local `bin/fm-test-run.sh` measurement of 962 ms, added because the CI job that would have recorded it was cancelled at the 15-minute ceiling before writing an artifact.
A script with no hint gets the conservative `PORTABLE_SERIAL_DEFAULT_WEIGHT_MS` default.
Hints only affect balance: the coverage guard keeps the partition complete and disjoint whatever they say, so a stale hint costs a slower shard rather than lost coverage.
The table below is the current LPT assignment, including those defaults; it is not a full artifact refresh.
Many serial scripts still lack a measured hint, so CI wall time can run well above these sums.

| Lane | Script count | Estimated duration |
|---|---:|---:|
| `portable-serial-1of4` | 26 | 557186 ms (~557.2 s) |
| `portable-serial-2of4` | 33 | 557185 ms (~557.2 s) |
| `portable-serial-3of4` | 34 | 557187 ms (~557.2 s) |
| `portable-serial-4of4` | 33 | 557185 ms (~557.2 s) |
| imbalance | | 2 ms |
...
| portable serial 1-4 | job `timeout-minutes: 15` | Estimated LPT sums are about 9.3 minutes, and CI wall time already runs hotter, so remaining margin under this cap is thin. |
