...
Missing, duplicate, malformed, backend-inconsistent, or task-mismatched endpoint records are preserved and refused. An exactly empty `window=` record means the endpoint already disappeared and is accepted for non-scout, non-secondmate ship tasks when the remaining cleanup-critical identity validates.
...
FM_AGENT_ARCHIVES_ROOT=       # optional backup-mirror root for archived task artifacts (default in bin/fm-archive-task.sh); see that script's header
