...
It deliberately leaves `commands.test` unset: no-mistakes Test remains intent-targeted rather than using a changed-file test baseline, and `.github/workflows/ci.yml` owns broad regression across the behavior, platform, security, Herdr, tmux, lifecycle, and compatibility lanes.
...
The no-mistakes Test command is intent-targeted and must not become a complete `tests/*.test.sh` walk; the runner's `--help` output owns the exact local selection, scheduling, and timeout rules.
