...
Missing, duplicate, malformed, backend-inconsistent, or task-mismatched endpoint records are preserved and refused. An exactly empty window= record (endpoint already gone) is accepted for non-scout, non-secondmate ship tasks when the remaining cleanup-critical identity validates.
