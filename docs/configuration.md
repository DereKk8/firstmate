...
`config/startup-memory-budget` is the primary-authoritative per-home advisory review threshold for the startup prompt-memory surface: `data/captain.md`, `data/captain-shared.md`, and `data/learnings.md` together.
...
Use `bin/fm-startup-memory-budget.sh read` to validate and print the effective value, or `bin/fm-startup-memory-budget.sh report` to account for the three files and report whether curation is recommended.
...
The weekly `/syncfirstmate` check reports a curation recommendation without writing, and `/reset-window` requires `/stow` to perform the complete curation pass before handoff.
The internal `/stow` skill curates only the editable local files in that case and reports the primary-owned shared file as a concrete exception if it alone exceeds the advisory threshold.
...
`AGENTS.md` section 4 owns the dispatch and array-selection procedure.
