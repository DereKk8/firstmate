...
config/extensions.d/  explicit trusted external process-event-adapter bindings, one mode-0600 JSON per package; LOCAL, gitignored; absent = inert, no package or state path (docs/extension-bindings.md, docs/configuration.md "Trusted external process-event adapters")
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  projects.md        thin fleet navigation registry recording each project's standing delivery posture and optional branch template; firstmate-private, parsed by fm-project-mode.sh for mechanical sync, seeding, and ship-brief branch templates (section 6)
  secondmates.md      local and remote secondmate routing table; firstmate-private, maintained by the secondmate seed helpers (section 6)
  extensions/         content-addressed trusted external package trees (packages/<id>/<version>/<digest>/ plus staging, retired-bindings, and retired-staging); LOCAL, gitignored; written and validated only by bin/fm-extension.mjs (docs/extension-bindings.md)
...
state/               runtime records and signals; gitignored
  ...
  home-summary.json   atomically published bounded home summary ledger with freshness epoch metadata, refreshed after locked session start, status change, spawn, teardown, and on a live-watcher cadence; bin/fm-home-summary-refresh.sh owns publication, and state/.home-summary-refresh.log is its bounded error log
  extensions/         enabled trusted external process-event adapter working namespaces; written only by bin/fm-extension.mjs (docs/extension-bindings.md)
...
When a secondmate is created for a domain, hand its in-scope queued main-backlog items into its home with `bin/fm-backlog-handoff.sh <secondmate-id> <item-key>...` so it owns that domain's queue from day one; never hand off `local-only` items.
...
When the captain invokes `/explain`, load the `explain` skill to plainly explain pending decisions waiting on the captain or a named decision or topic.
...
Always review the diff with `bin/fm-review-diff.sh <id>` rather than eyeballing a pooled clone, which can lag `origin`; its default-branch base and PR-head preference are defined in [docs/architecture.md](docs/architecture.md#delivery-modes-are-explicit-per-task).
...
- While `state/.afk` exists, the daemon owns supervision; do not arm a separate watcher.
  The daemon is never launched on Pi or pi-signed, where the ordinary supervision session continues under the record with main parked: the branch takes every safe actionable wake it can, and only a declined wake (including a broken branch or unsafe scan) or a watcher failure wakes main.
...
**Firstmate is harness-agnostic.** Work can be delegated to any verified harness; a worker that determines a subtask would be significantly more effective on a different harness should suggest it via `needs-decision` and await approval, never delegating to another harness autonomously.
