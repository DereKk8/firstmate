...
- `TANGLE: <remediation>` - the primary checkout is tangled: stranded on a feature branch instead of its default branch, or holding uncommitted changes to tracked files; `AGENTS.md` section 8 explains why this guard exists and what it protects.
  The work is safe; restore the primary per the printed remediation: for a branch tangle, `git -C <root> checkout <default>` then re-validate that branch in a proper worktree; for dirty tracked files, preserve with `git -C <root> switch -c <branch> && git -C <root> commit -a -m wip` or `git -C <root> stash`.
  This is the only sanctioned firstmate-initiated git write to the primary, and it is a non-destructive operation that strands nothing.
...
- Fleet sync via `bin/fm-fleet-sync.sh` (set `FM_FLEET_PRUNE=0` to temporarily disable branch pruning); the refresh is bounded by `FM_FLEET_SYNC_BOOTSTRAP_TIMEOUT` and a timeout is a `FLEET_SYNC` skip, never a startup blocker.
- Secondmate home fast-forward: every live secondmate home is fast-forwarded to firstmate's own current default-branch commit - a purely local fast-forward that never touches gitignored operational dirs; a dirty, diverged, or in-flight home is skipped.
- Secondmate liveness: each live secondmate's endpoint is probed for a real agent process and respawned only on a confident dead reading, reported as `SECONDMATE_LIVENESS:` lines.
- Inheritable config propagation: pushes `config/crew-dispatch.json`, `config/crew-harness`, and `config/backlog-backend` into each live secondmate home's `config/` (never `config/secondmate-harness`).
- X-mode artifact writes: with a non-empty `FMX_PAIRING_TOKEN` in `.env`, creates `state/x-watch.check.sh` and `config/x-mode.env`; on opt-out, deletes them.

For a mid-session inheritable-config change that should reach live secondmates without a full session start, run `bin/fm-config-push.sh`; it reports `pushed`, `unchanged`, `skipped`, or `error` per item per home.
