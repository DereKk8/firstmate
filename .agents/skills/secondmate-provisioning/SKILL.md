...
A bare `<harness>` (today's format, e.g. `<adapter-name>`) behaves exactly as before - harness only, no model/effort flag - so this is fully backward-compatible.
